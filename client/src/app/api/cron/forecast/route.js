...
export async function POST(req) {
  ...
  if (locErr)
    return NextResponse.json(errorHandler(locErr, errorCodes.SUPABASE_ERROR), {
      status: 500,
    });
  if (!locations?.length)
    return NextResponse.json(
  ...
  for (const loc of locations) {
    ...
    if (targetDateInput) {
      ...
      if (!parsed.isValid)
        return NextResponse.json(
          errorHandler(new Error("Invalid date input"), errorCodes.ZOD_ERROR),
          { status: 400 }
        );
      targetDayTZ = parsed.startOf("day");
    } else {
      // default: inclusive 7-day window starting today
      targetDayTZ = DateTime.utc().setZone(tz).plus({ days: 6 }).startOf("day");
    }
    ...
    const daysSpan = Math.floor(targetDayTZ.diff(startDayTZ, "days").days) + 1;
    ...
    for (const lot of lots) {
      ...
      for (
        ...
        const slots = [];
        for (let t = startTZ; t < endTZ; t = t.plus({ minutes: intervalMin })) {
