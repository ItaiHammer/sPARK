...
export default function GarageCard({ garage, order }) {
  ...
  // animate occupancy %
  useEffect(() => {
    const controls = animate(0, garage.point, {
      duration,
      onUpdate(latest) {
        setOccupancyValue(Math.floor(latest));
      },
    });

    return () => controls.stop();
  }, [garage.point, duration]);

  // animate spot count
  useEffect(() => {
    const targetSpots = Math.floor((garage.point / 100) * garage.spot_count);

    const controls = animate(0, targetSpots, {
      duration,
      onUpdate(latest) {
        setSpotCount(Math.floor(latest));
      },
    });

    return () => controls.stop();
  }, [garage.point, garage.spot_count, duration]);
